  ...
  if (posts.length === 0) {
    return (
      <div className="rounded-xl border border-dashed border-border py-16 text-center text-ink-soft">
        No posts yet.{" "}
        <Link href="/blog/admin/posts/new" className="text-accent-deep hover:underline">
          ...
          {posts.map((post) => (
            <tr key={post.id} className="border-b border-border last:border-0 hover:bg-paper-card/60">
              ...
              <td className="px-5 py-3.5 text-ink-soft">
                {post.categories.map((c) => c.category.name).join(", ") || "N/A"}
              </td>
              ...
              <td className="px-5 py-3.5">
                <div className="flex items-center justify-end gap-3 text-ink-faint">
                  {post.status === "published" && (
                    <Link href={`/blog/posts/${post.slug}`} target="_blank" className="hover:text-ink" title="View">
                      <ExternalLink size={16} />
                    </Link>
                  )}
                  <Link href={`/blog/admin/posts/${post.id}/edit`} className="hover:text-ink" title="Edit">
                    <Pencil size={16} />
                  </Link>
                  <button
