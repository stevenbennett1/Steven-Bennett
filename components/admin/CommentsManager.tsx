...
export default function CommentsManager({ initial }: { initial: AdminComment[] }) {
  ...
  async function setStatus(id: string, status: "approved" | "pending") {
    const res = await fetch(`/blog/api/admin/comments/${id}`, {
      method: "PATCH",
  ...
  async function remove(id: string) {
    ...
    const res = await fetch(`/blog/api/admin/comments/${id}`, { method: "DELETE" });
          ...
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              ...
              <p className="mt-1 text-xs text-ink-faint">
                on{" "}
                <Link href={`/blog/posts/${c.post.slug}`} target="_blank" className="hover:text-accent-deep">
                  {c.post.title}
                </Link>{" "}
