    ...
    <div className="mx-auto max-w-6xl px-5 sm:px-8">
      ...
      {categories.length > 0 && (
        <div className="flex flex-wrap gap-x-6 gap-y-2 border-b border-border py-6">
          {categories.map((c) => (
            <Link key={c.slug} href={`/blog/category/${c.slug}`} className="text-sm text-ink-soft hover:text-ink">
              {c.name}
            </Link>
          ))}
        </div>
        ...
        <section id="latest" className="scroll-mt-24 pb-20 pt-12">
          ...
          {posts.length >= 13 && (
            <div className="mt-12 flex justify-center">
              <Link href={`/blog/category/${categories[0]?.slug ?? ""}`} className="btn-outline">
